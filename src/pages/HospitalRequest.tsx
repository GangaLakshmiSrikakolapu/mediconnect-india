...
import { CheckCircle, Plus, Trash2, UserPlus, ArrowRight, ArrowLeft, Loader2, Building2 } from 'lucide-react';
...
const HospitalRequest = () => {
  ...
  const addDoctor = () => {
    if (!currentDoctor.doctor_name.trim() || !currentDoctor.email.trim() || !currentDoctor.specialization) {
      toast({ title: 'Fill required doctor fields (name, email, specialization)', variant: 'destructive' });
      return;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('hospital-request', {
        body: {
          hospital_name: form.name, email: form.email, phone: form.phone,
          state: form.state, district: form.district, address: form.address,
          specializations: specs, upi_qr_url: qrUrl,
          doctors: doctors.map(d => ({ doctor_name: d.doctor_name, age: d.age, email: d.email, phone: d.phone, specialization: d.specialization, education: d.education, experience: d.experience })),
        },
      });
    ...
    return (
      <div className="min-h-[calc(100vh-4rem)] bg-secondary/30 flex items-center justify-center">
        <div className="text-center py-16 animate-fade-in">
          ...
          <h2 className="font-heading text-2xl font-bold mb-2">Hospital Request Submitted!</h2>
          <p className="text-muted-foreground">Your request will be reviewed by our admin team. {doctors.length} doctor(s) submitted.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-[calc(100vh-4rem)] bg-secondary/30">
      <div className="container py-8 max-w-2xl">
        ...
        <div className="flex items-center justify-center gap-2 mb-8">
          {['Hospital Info', 'Add Doctors', 'Review'].map((label, i) => {
            const idx = ['hospital', 'doctors', 'review'].indexOf(step);
            return (
        ...
        {step === 'hospital' && (
          <Card className="border-0 card-shadow">
            ...
            <CardContent>
              <form onSubmit={e => { e.preventDefault(); if (!form.name || !form.email || !form.phone || !form.state || !form.district || !form.address || specs.length === 0) { toast({ title: 'Fill all required fields & select specializations', variant: 'destructive' }); return; } setStep('doctors'); }} className="space-y-4">
                ...
                <Button type="submit" className="w-full rounded-xl" size="lg">Next: Add Doctors <ArrowRight className="h-4 w-4 ml-2" /></Button>
              </form>
            </CardContent>
          </Card>
        )}
                  ...
                  {doctors.map((d, i) => (
                    <div key={i} className="flex items-center justify-between p-3 rounded-xl bg-secondary/50">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">{d.specialization} · {d.experience} yrs · {d.email}</p>
                      </div>
          ...
          <div className="space-y-4">
            <Card className="border-0 card-shadow">
              ...
              <CardContent className="grid grid-cols-2 gap-3 text-sm">
                ...
                <div><span className="text-muted-foreground block text-xs">Email</span>{form.email}</div>
            ...
            <div className="flex gap-3">
              ...
              <Button className="flex-1 rounded-xl" size="lg" onClick={handleSubmit} disabled={loading}>
                {loading ? <><Loader2 className="h-4 w-4 mr-2 animate-spin" />Submitting...</> : 'Submit Hospital Request'}
              </Button>
            </div>
          </div>
        )}
